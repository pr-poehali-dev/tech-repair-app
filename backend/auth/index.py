"""Аутентификация: login, logout, me, profile — action передаётся через ?action="""
import json
import os
import hashlib
import secrets
...
def get_user_by_session(conn, session_id: str):
    ...
    if not row:
        return None
    return {'id': row[0], 'name': row[1], 'email': row[2], 'role': row[3], 'phone': row[4], 'speciality': row[5], 'rating': float(row[6] or 5)}
...
def handler(event: dict, context) -> dict:
    ...
    if action == 'profile' and method == 'PUT':
        ...
        for f in ['name', 'email', 'phone']:
            if body.get(f):
                cur.execute(f"UPDATE {SCHEMA}.users SET {f}=%s WHERE id=%s", (body[f], uid))
        conn.commit()
